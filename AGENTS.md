...
6. Restart Codex — run `/mcp` to confirm both `scopus` and `zotero` servers show as connected

**Note:** MCP server definitions belong in `.mcp.json` (project root), not in `.Codex/settings.local.json`. The `enableAllProjectMcpServers: true` flag in `.Codex/settings.local.json` auto-approves them on load.
